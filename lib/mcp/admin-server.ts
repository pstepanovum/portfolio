...
import { registerGmailReadTools, registerGmailWriteTools } from "@/lib/mcp/tools/gmail";
import { GMAIL_SCOPE_READ, GMAIL_SCOPE_WRITE, hasScope } from "@/lib/oauth/config";
...
parameter may be omitted. Tools that send mail are irreversible and appear only
when the connection was granted gmail:write; confirm recipients and wording with
the user before sending. If a tool reports that an account needs reconnecting,
say so plainly: the fix is on the dashboard, not in another tool call.`;
 ...
 */
export function buildAdminMcpServer(scopes: string[]) {
